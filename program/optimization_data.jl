...
node_reactive_power = [length(indexes) > 0 ? sum(generator_reactive_ub[i] for i in indexes) : 0 for indexes in node_generators] # Calculate --> net flow of reactive 
                                                                                                                                #power flowing into each node
